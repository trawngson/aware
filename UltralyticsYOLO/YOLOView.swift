...
public class YOLOView: UIView, VideoCaptureDelegate {
  ...
  /// Pauses the camera session (stops the camera feed and green indicator)
  public func pauseSession() {
    videoCapture.stop()
  }
  
  /// Resumes the camera session
  public func resumeSession() {
    videoCapture.start()
  }
  
  /// Returns whether the camera session is currently running
  public var isSessionRunning: Bool {
    return videoCapture.captureSession.isRunning
  }
  
  /// Captures the current camera frame as UIImage (without overlays)
  public func captureCurrentFrame() -> UIImage? {
    return videoCapture.captureCurrentFrame()
  }
